...
--creating the table for the movies--
CREATE TABLE movies (
...
-- How can you calculate the average age of male customers in the "customers" table?--
SELECT AVG(age) FROM customers WHERE gender = 'Male';

-- How can you retrieve the top 4 movies with the highest ratings from the "movies" table?--
...
SELECT title , 
rating FROM movies WHERE rating > 8.5;
...
SELECT c.name FROM customers c JOIN bookings b ON c.customer_id = b.customer_id 
GROUP BY c.customer_id HAVING COUNT(*) > 2;

-- How can you calculate the percentage of male customers in the "customers" table
SELECT AVG(CASE WHEN gender = 'Male' THEN 1 ELSE 0 END) * 100 FROM customers;

-- How can you retrieve the customer who has made the most bookings?
SELECT c.name, COUNT(*) AS booking_count FROM customers c  JOIN bookings b ON c.customer_id = b.customer_id 
GROUP BY c.customer_id ORDER BY booking_count DESC LIMIT 1;

-- Which SQL query can be used to list the names and ages of customers 
-- who have booked tickets for the movie with movie_id = 5?
SELECT c.name, c.age FROM customers c 
JOIN bookings b ON c.customer_id = b.customer_id WHERE b.movie_id = 5;

-- Given the Customers, Movies and Bookings tables, write an SQL query to retrieve the count of bookings for each genre.
...
-- Retrieve the customer ID, name, 
-- and assign an age group based on the customer's age using the CASE statement.
...
-- Calculate the total revenue generated by the cinema hall based on the cost of 
-- bookings between 4th June 2023 to 12th June 2023 as total_revenue.
...
-- Calculate the total cost of all bookings made by each customer as total_cost 
-- and display the customer's name along with the total cost. Group the results by customer_id.
...
SELECT m.title AS title,
       (SELECT COUNT(*) 
        FROM bookings b 
        WHERE b.movie_id = m.movie_id) AS total_bookings
FROM movies m;

-- Find the customer id and name of the customer who made the first booking in the database.
-- Approach 1: Using a Subquery to Determine the Earliest Booking Date
SELECT c.customer_id, c.name
FROM bookings b
JOIN customers c ON b.customer_id = c.customer_id
WHERE b.booking_date = (SELECT MIN(booking_date) FROM bookings);

-- Approach 2: Using ORDER BY with LIMIT to Retrieve the First Record Precisely

SELECT c.customer_id, c.name
FROM bookings b
JOIN customers c ON b.customer_id = c.customer_id
ORDER BY b.booking_date, b.timings
LIMIT 1;
...
-- Retrieve the customer names and the number of bookings made by each customer 
-- as booking_count, sorted in descending order of the booking count.

-- 1.Using a Correlated Subquery
SELECT
    name,
    (SELECT COUNT(*) FROM bookings WHERE bookings.customer_id = customers.customer_id) AS booking_count
FROM
    customers
WHERE
    (SELECT COUNT(*) FROM bookings WHERE bookings.customer_id = customers.customer_id) > 0
ORDER BY
    booking_count DESC;
    --or
...
WITH customer_tickets AS (
    SELECT customer_id, SUM(ticket_quantity) AS total_tickets
    FROM bookings
    GROUP BY customer_id
)
...
WHERE ct.total_tickets = (
    SELECT MAX(total_tickets) FROM customer_tickets
);